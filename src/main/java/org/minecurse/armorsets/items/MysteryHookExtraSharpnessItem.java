...
public class MysteryHookExtraSharpnessItem extends ClickItem {
   public String getName() {
      return "mystery_diamond_hook_extra_sharpness";
   }

   public ItemStack createItem() {
      return new ItemBuilder(Material.DIAMOND_HOE)
         .name("&8» &b&lThe Diamond Hook &3(+1 Sharpness) &8«")
         .enchantment(Enchantment.DAMAGE_ALL, 6)
         .lore("")
         .lore(
            new String[]{
   ...
   public boolean onClick(Player p0, ItemStack p1, Action p2) {
      ...
      if (p2.name().contains("RIGHT")) {
         ...
         ItemStack item = DiamondHook.buildItem(chosen, 7, 6);
         p0.sendMessage(
            StringUtil.color(
