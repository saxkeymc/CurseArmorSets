...
public class MysteryHookExtraDamageItem extends ClickItem {
   public String getName() {
      return "mystery_diamond_hook_extra_damage";
   }

   public ItemStack createItem() {
      return new ItemBuilder(Material.DIAMOND_HOE)
         .name("&8» &b&lThe Diamond Hook &3(+1 Damage) &8«")
   ...
   public boolean onClick(Player p0, ItemStack p1, Action p2) {
      ...
      if (p2.name().contains("RIGHT")) {
         ...
         ItemStack item = DiamondHook.buildItem(chosen, 8, 5);
         p0.sendMessage(
            StringUtil.color(
